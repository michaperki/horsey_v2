...
- **Rating volatility display.** Visible +Δ / −Δ after settlement, with tier-appropriate dramatization (verified high-stake matches feel weightier — heavier font, stronger color, longer settle). Status: **partial** — number ships with a slide-from-above + bump reveal (600ms delay, 700ms animation) that lands after the amount counter completes; tier-keyed dramatization (heavier on verified/high-stake) is the remaining pending piece.
- **Rematch invitation timing.** Rematch CTA appears after settlement animation finishes (~1.2s gate), not before. Status: **done** — `.settlement-rematch-gate` fades the rematch + find-new-opponent CTAs in at 1.2s after settlement renders, gated to once per gameId so realtime-triggered re-renders don't replay the animation. Reduced-motion fallback ships.
- **Loss bankroll dip.** The available balance ticks down at settlement so losses have a visible counter movement (escrow leaves at game start, so without this the loss has no balance animation). Bankroll-tick SFX descends in pitch on a downward tween. Status: **done**.

...
- **Avatar identity and cosmetics.** The runtime avatar renderer and dev composition canvas are now in motion, but the next product leverage is not shop/catalog breadth. Near-term work should make avatar identity real: base piece reflects rating class, border reflects trust tier, adornments reflect earned history/live state. Priority order is: lock trust-border ladder mapping, implement base-piece resolver, add Profile explanation, persist cosmetic ownership/equip state, wire first-win laurel grant, wire trust-tier border grants, then improve editor sorting/recipe support. Item shop remains deferred until those identity loops exist. See `docs/COSMETICS_FORMALIZATION.md` § 5.0 and § 8.

...
Payments v1: users can buy non-cashout entertainment chips through NOWPayments hosted stablecoin invoices. Checkout and signed IPN webhook code are wired: webhooks append `purchase` ledger entries to the existing play-token balance idempotently when NOWPayments reaches `finished`. Profile/Cashier shows package tiles and purchase rows; ToS acceptance, kill switch, catalog, and cashout waitlist are in place. Live use still needs provider secrets, `HORSEY_PAYMENTS_ENABLED=1`, and focused payment tests/live-provider verification before enabling. Refunds, spend caps, and real geo enforcement remain follow-ons. See `PAYMENTS_NEXT_PASS.md`.
...
4. **Payment readiness polish.** NOWPayments invoice creation + signed IPN webhook + idempotent chip credit are code-side wired, but the path still needs focused automated tests, provider-account/live IPN verification, and docs/secrets checklist before flipping `HORSEY_PAYMENTS_ENABLED=1`.

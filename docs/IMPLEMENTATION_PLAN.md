...
- **Avatar identity and cosmetics.** *(Ripped 2026-05-25.)* The v1 cosmetic system (PNG-layered renderer, dev composition canvas, trust-border grants, first-win laurel grant, base-piece resolver, ownership/equip persistence) was removed from the codebase. The v1 thinking docs are preserved at `docs/archive/COSMETICS_FORMALIZATION.md`, `docs/archive/COSMETICS_NEXT_PASS.md`, `docs/archive/COSMETICS_INVENTORY_AUDIT.md`, and the original PNG assets are at `scripts/reference/cosmetics-v1/assets/`. The avatar surface is currently a baseline initial-letter span across every render site; the new cosmetic system will introduce its own renderer, schema, and asset pipeline when designed. The avatar-semantics principle in `PROJECT_SOUL.md` survives the rewrite.

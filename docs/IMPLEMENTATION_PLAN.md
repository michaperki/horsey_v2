...
Today: admin has moved beyond read-only and through a console redesign (`docs/ADMIN_CONSOLE_NEXT_PASS.md`, shipped). Schema v11's `users.is_admin` gates `/api/admin/*` and the `#admin` web page. The console is a felt operator terminal: an **Overview triage landing** (`/api/admin/overview` — live / open-reports / stuck / pending-analysis / restricted counts + escrow held + actionable flags), intent-grouped **TRUST / MONEY / OPS** nav, and a first-class **Fair-Play Queue** (`/api/admin/analysis`) feeding an in-place breadcrumb drilldown into the per-game analysis (phase / critical / clock / engine-rank evidence). Covers Overview / Reports / Fair-Play Queue / Restrictions / Ledger / Purchases / Games / Stuck / Challenges / External / Users / Audit. The admin mutation slice ships void / adjust / restrict / clear-restriction with `admin_actions` audit rows and append-only ledger corrections. Starting balances are still seed-grant-only; manual non-game balance correction is not yet an in-app tool.
Real version remaining: broader dispute workflow, dual-control for high-risk mutations, and non-game balance correction tooling. **Paused (2026-05-29):** admin/fair-play surface area is ahead of need — further admin investment waits until there are real users generating volume to triage.
...
Payments v1: users can buy non-cashout entertainment chips through NOWPayments hosted stablecoin invoices. Checkout and signed IPN webhook code are wired with focused tests for signature verification, idempotent crediting, and viewer-scoped reads. Admin Purchases tab + `/api/admin/purchases` give operators a live view. Profile/Cashier shows package tiles and purchase rows; ToS acceptance, kill switch, catalog, and cashout waitlist are in place. Live use still needs provider secrets, `HORSEY_PAYMENTS_ENABLED=1`, and a small live-provider smoke per `PAYMENTS_NEXT_PASS.md` § Go-live checklist. Edge geo-block wiring, refunds, and spend caps remain follow-ons.
...
- **Read-only admin slice (Phase 6 first cut).** Status: **done** — schema v11 adds `users.is_admin` (hand-set in the DB via `UPDATE users SET is_admin=1 WHERE handle='...'`; no admin-creates-admin UI yet). `/api/admin/{users,games,stuck-games,ledger,challenges,external-accounts}` were the first read-only surface and are gated on `is_admin`. Later slices added Reports, Audit, and guarded mutations.
- **Report-player path.** Status: **done** — schema v15 adds `reports`; `POST /api/reports` lets players report a target player and/or a game they participated in; `/api/admin/reports` and `/api/admin/reports/:id/status` provide the first admin inbox/status workflow. Seeds Phase 6 anti-cheat ingestion. Policy reference: `OPERATIONAL_POLICY.md` § 5.2 (user dispute flow).
- **Admin mutation slice — void / adjust / restrict.** Status: **done** — policy `§ 8` requires "ability to manually void/refund/adjust a match" + freeze accounts before first real-money users. **Locked 2026-05-28: all match outcomes are reversible by admin discretion** — no rules on *what* can be reversed, only audit-trail rules on *how*. **Void-not-refund framing (2026-05-28):** void is a *state*, refund is its ledger *consequence* — voiding a game writes the compensating entries that return both stakes. There is no separate `/refund` endpoint; if you want stakes returned, you void. The slice adds an `admin_actions` audit table (actor, target, action, reason, before/after JSON), a `user_restrictions` table (separate rows per restriction, audit-friendly, indexed for matchmaking/withdrawal-review queries), a reason field on every mutation, and endpoints for game void, game adjust, applying restrictions, and clearing restrictions. The restrict endpoint takes the *full* shadow-restriction ladder from `FAIR_PLAY_NEXT_PASS.md` § Enforcement Ladder. Hard ban locks the user out *and* auto-voids any live game they're in (opponents refunded via the same void path). Future void triggers (server-outage detection, unrecoverable-state detection) are out of slice 2 but the `state='voided'` shape supports them.
...
- **Buy Chips panel.** Status: **done** — Profile -> Buy Chips renders the package tiles. Tiles are locked when the kill switch is off or the viewer is geo-blocked. When enabled, Buy hits `POST /api/payments/checkout`, creates a NOWPayments invoice, and redirects to the hosted invoice URL.
...
- **Purchase ledger.** Status: **code-side done, needs payment-focused tests/live verification** — `purchases` table tracks `provider_session_id`, `provider_payment_id`, package, USD amount, chips credited (with bonus), status, pay currency, ledger entry id, raw IPN payload for audit. Signed IPNs idempotently credit `purchase` ledger entries once status maps to `finished`.
- **NOWPayments wire-up.** Status: **code-side done, not enabled by default** — `POST /api/payments/checkout` creates a NOWPayments invoice via `POST https://api.nowpayments.io/v1/invoice` and returns the hosted `invoice_url`. `POST /api/payments/webhook` verifies HMAC-SHA512 IPN signatures against the merchant IPN secret and idempotently credits chips when status reaches `finished`. Live use needs `NOWPAYMENTS_API_KEY` + `NOWPAYMENTS_IPN_SECRET` via `fly secrets set` and `HORSEY_PAYMENTS_ENABLED=1`.
...
Status: **code-side wired, operationally dark** — checkout/webhook code exists behind the kill switch. Canonical detail lives in `PAYMENTS_NEXT_PASS.md` and ADR 0007.
...
- Report player/game flow. Status: **done** — player intake, `reports` table, admin inbox, and admin status changes are shipped.
- Admin/support views for users, games, ledger, escrow, settlements, reports, fair-play review. Status: **mostly done** — the admin **console** (see `docs/ADMIN_CONSOLE_NEXT_PASS.md`) ships a felt operator terminal: Overview triage landing, TRUST/MONEY/OPS grouped nav, a first-class Fair-Play Queue with status chips, and the per-game analysis drilldown. Void / adjust / restrict mutations and the reports inbox/status flow are shipped. Dual-control for high-risk mutations and non-game balance correction tooling remain pending (mock #8). **Paused 2026-05-29 until there are users.**
...
- Manual game settlement correction workflow with audit trail. Status: **done** for game void/adjust. Non-game balance correction tooling remains pending.
...
3. **Report-player path (Bucket B).** Reports table + intake endpoints + admin read/status surface. Status: **done** — schema v15, `POST /api/reports`, Admin Reports tab, and focused API tests.
4. **Payment readiness polish.** Status: **code-side done** — `tests/payments.test.mjs` covers checkout failure modes, HMAC-SHA512 IPN signature verification (good/tampered/missing/no-secret/canonical-form), `mapNowPaymentsStatus` mapping, idempotent crediting on `finished` (replay-safe), non-`finished` status handling, viewer-scoped purchase reads, and admin-gated `/api/admin/purchases`. Admin Purchases tab is wired. Remaining steps are operational, not code: NOWPayments merchant account + IPN secret + Fly secrets + live small-invoice smoke + edge geo-block wiring. See `PAYMENTS_NEXT_PASS.md` § Go-live checklist.

...
Today: admin has moved beyond read-only. Schema v11's `users.is_admin` gates `/api/admin/*` and the `#admin` web page; the portal covers Users / Games / Reports / Stuck / Ledger / Challenges / External / Audit. The admin mutation slice ships void / adjust / restrict / clear-restriction with `admin_actions` audit rows and append-only ledger corrections. Starting balances are still seed-grant-only; manual non-game balance correction is not yet an in-app tool.
Real version remaining: broader dispute workflow, richer fair-play review, dual-control for high-risk mutations, and non-game balance correction tooling.
...
- Admin/support views for users, games, ledger, escrow, settlements, reports, fair-play review. Status: **partial** — admin portal now covers users / games / reports / stuck-games / ledger / challenges / external-accounts / audit. Void / adjust / restrict mutations and the first reports inbox/status flow are shipped. Richer fair-play review, dual-control, and non-game balance correction tooling remain pending (mock #8).

...
2. **Admin mutation + audit slice (Bucket B).** `admin_actions` audit table (actor / target / action / reason / before-after JSON), `user_restrictions` table, the three mutation endpoints (`void` / `adjust` / `restrict` — void is the state, refund is its ledger consequence), and the full shadow-restriction ladder from `FAIR_PLAY_NEXT_PASS.md` § Enforcement Ladder shipped in one slice. Hard ban auto-voids any live game the banned user is in. Status: **pending**.

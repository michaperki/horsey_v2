...
2. **Admin mutation + audit slice (Bucket B).** `admin_actions` audit table (actor / target / action / reason / before-after JSON), the four mutation endpoints (`void` / `refund` / `adjust` / `restrict`), and the full shadow-restriction ladder enum from `FAIR_PLAY_NEXT_PASS.md` § Enforcement Ladder shipped in one slice. Hard ban included but expected to be rare. Status: **pending**.

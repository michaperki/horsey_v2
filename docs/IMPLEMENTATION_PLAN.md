...
Real version remaining: per-tab visibility throttling; the existing `parseTimeControl` regex enforces `min+inc` in whole minutes — sub-minute formats (e.g. "30s+0" bullet) need a format extension. (Drift smoothing landed: the client now anchors on `performance.now()` at message receipt and ticks via `requestAnimationFrame`, so the display is monotonic between server updates instead of recomputing from `Date.parse(lastMoveAt)` each frame.)
Phase: **4** core done; sub-minute time controls + per-tab throttling pending.

...
Deliverables:
- Realtime connection layer for game state, challenge notifications, presence, clocks, quick chat. Status: **partial** — WebSocket transport + broker + `user:*` / `game:*` channels + opponent move propagation + finalize push are **done** (mock #3, ADR 0004). Presence, clocks, and quick chat are deferred future consumers of this layer.

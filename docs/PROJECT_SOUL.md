...
**Avatar semantics.** *(Note: the v1 cosmetic system was ripped 2026-05-25 and is being rebuilt from scratch. The principle below survives the rewrite and should guide the new system when it lands.)* The avatar is public identity, not a generic skin doll. Base chess piece communicates chess-strength / rating class; frame communicates trust and account status; adornments communicate earned history, rewards, live state, stable identity, or shop-safe expression. Do not sell or grind-gate base-piece upgrades, and do not let adornments impersonate trust. The archived v1 thinking lives in `docs/archive/COSMETICS_FORMALIZATION.md`.

...
- **Current priority update, 2026-05-27.** Before widening the product surface again, do the full mobile pass. The next conceptual/product pass after that is fair-play/admin trust: what it takes to catch engine use or human assistance, which metrics belong in admin first, and which summary signals may eventually graduate into user history, HUD, and profile surfaces. See `docs/FAIR_PLAY_NEXT_PASS.md`.
- **Cashout model reopened, 2026-05-27.** The no-cashout entertainment-chip ToS is the current implemented posture, but the strategic target may shift toward a dual-currency / sweepstakes-compatible model to make redemption plausible sooner. Treat this as active discovery requiring legal review, not as permission to imply redeemability in product copy before the rules, currency split, and jurisdiction gates are written down.
